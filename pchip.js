...
var PCHIP = {
	...
	**/
		...
		dest = new Array(); //may be slow because we are pushing new elements
		offset = 0;
		for( var i=0; i<source.length-1; i++){
			//fill dest array with sucessive pchip'd points 
			var thisPoint = source[i];
			var nextPoint = source[i+1];
			offset += this.chip_v(thisPoint, nextPoint, [ m[i], m[i+1] ], frequency, dest, offset); //faster?
			//offset += this.chip_p(thisPoint[1],nextPoint[1],thisPoint[0],nextPoint[0],m[i],m[i+1],frequency,dest,offset);
		}
		return dest;
	},
	...
	slopes_monotone: function(source){
		...
		slope[0] = (source[1][1]-source[0][1])/(source[1][0]-source[0][0]+smallVal);
		m[0] = slope[0];
		if(slope[0] == 0){
			to_fix.push(0);
		}
		...
		for( var i=1; i<inLen-1; i++){
			...
			//slope[i] = (y1-y0)/dx;
			slope[i] = (source[i+1][1]-source[i][1])/(source[i+1][0]-source[i][0]+smallVal);
	...
	slopes_shape_preserving: function (pts){
		...
		m = new Array(pts.length); // array for new slopes
		...
		for(i=1; i<pts.length-1; i++){
			...
			if( slope0 == 0.0 || slope1 == 0.0 ){ //don't overshoot
				m[i] = 0.0;
			}else if( this.sgn(slope0) != this.sgn(slope1) ){ //local min/max
				m[i] = 0.0;
			}else{
	...
	sgn : function (x){
		/** the signum [http://en.wikipedia.org/wiki/Sign_function] function or sign of x **/
		if( x < 0.0 ){
			return -1;
		}
		if ( x > 0.0){
		...
		return x; //when x == 0 or not a number
	},

	chip_p : function (yi,yii,xi,xii,mi,mii,freq,resultArray,offset){
	/** This is the Cubic Hermite Interpolating Polynomial (chip) function taking *Point* values as arguments:
	 *  yi, and yii are consecutive y values in a plot (points between which to interpolate
	 *  xi, and xii are consecutive x valuse associated with the above y values
	 *  mi, and mii are the slopes desired at each point
	 ...
	 *  numPts is returned to report the number of points added **/
		...
		var numPts = parseInt((xii-xi)*freq); //this could be zero if spacing is too small
		
		var a = 2.0/Math.pow(xii-xi,3);
		var b = (xii-xi)/2.0;
		var c = 1.0/Math.pow(xii-xi,2);
		
		for(var i=0; i<numPts; i++){
			var X = xi+i/freq;
			resultArray.push(Array(2));
			resultArray[offset+i][0] = X;
//			resultArray[offset+i][1] = yi *( 2.0/Math.pow(xii-xi,3)*Math.pow(X-xii,2)*(X-xi +(xii-xi)/2.0) )
//									  +yii*(-2.0/Math.pow(xii-xi,3)*Math.pow(X-xi ,2)*(X-xii-(xii-xi)/2.0) )
//									  + 1.0/Math.pow(xii-xi,2)*( mi*(X-xi)*Math.pow(X-xii,2)+mii*(X-xii)*Math.pow(X-xi,2) );
			resultArray[offset+i][1] = yi *( a*Math.pow(X-xii,2)*(X-xi +b) )
									  +yii*(-a*Math.pow(X-xi ,2)*(X-xii-b) )
									  + c *( mi*(X-xi)*Math.pow(X-xii,2)+mii*(X-xii)*Math.pow(X-xi,2) );
		}
		return numPts;	
	},
	...
	//	var b=yp1;
		...
		var numPts = parseInt(deltax*freq); //this could be zero if spacing is too small
	...
	//		resultArray[offset+i][1] = a+(x-x1)*(  b+(x-x1)*(  c+d*(x-x2)  ) );
			resultArray[offset+i][1] = y1+(x-x1)*(  yp1+(x-x1)*(  c+d*(x-x2)  ) );		
		}
		return numPts;	
	}
}
